...
@Aspect
@Component
public class SysLogAfterAspect {
	
	@Pointcut("@annotation(cn.bjtc.annotation.SysLoggerAfter)")
	public void doAspectAfter(){}
            ...
            if (method.getName().equals(methodName)) {    
               Class[] clazzs = method.getParameterTypes();    
                if (clazzs.length == arguments.length) {    
                	type = method.getAnnotation(SysLoggerAfter.class).type();    
                   break;    
               }    
           }    
       }    
	...
	private String getLogContent(JoinPoint joinPoint) throws ClassNotFoundException{
        ...
        for (Method method : methods) {    
            if (method.getName().equals(methodName)) {    
               Class[] clazzs = method.getParameterTypes();    
                if (clazzs.length == arguments.length) {    
                	content = method.getAnnotation(SysLoggerAfter.class).content();    
                   break;    
               }    
           }    
       }    
